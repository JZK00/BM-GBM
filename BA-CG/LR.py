...
def LR_method():
    ...
    ############### parameters select ##########
    classifier_default = LogisticRegression(random_state=seed)
    parameters_grid = [
        {"C": np.arange(1, 30, 1), "penalty": ["l2"], "solver": ["sag", "newton-cg", "lbfgs", "liblinear", "saga"],
         "max_iter": [10000]},
        #     {"C":  np.arange(1,30,1),"penalty": ["elasticnet"],"solver": ["saga"],"max_iter":[10000],"l1_ratio":np.arange(0,1)},
        {"C": np.arange(1, 30, 1), "penalty": ["l1"], "solver": ["liblinear", "saga"], "max_iter": [10000]}  #
    ]
    ...
    classifier = LogisticRegression(**tuned_model.best_params_)  # **tuned_model.best_params_C=2, max_iter=1000, penalty='l2',solver='sag'
